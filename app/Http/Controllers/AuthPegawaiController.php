...
class AuthPegawaiController extends Controller
{
    public function login(Request $request)
    {
        ...
        // $orang = MsOrang::where('no_ktp', $request->no_ktp)
        //     ->orderBy('last_sync', 'desc')
        //     ->first();

        // $query = DB::select("
        //     SELECT o.id, o.no_ktp, p.id_orang, o.last_sync
        //     FROM sdi.ms_orang o
        //     INNER JOIN sdi.ms_pegawai p ON p.id_orang = o.id
        //     WHERE o.no_ktp = ?
        ...
        //v.pegawai
        $queryPegawai = DB::select("
            SELECT *
            FROM sdi.v_pegawai
            WHERE no_ktp = ?
        ", [$request->no_ktp ?? null]);

        $orang = $queryPegawai[0] ?? null;
    ...
    public function me(Request $request)
    {
        ...
        $upk = DB::connection('mysql_sdi')->selectOne($sql, [$id_orang]);
        //dd($upk);
